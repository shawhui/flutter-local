...
import 'animator/animation_page_cover.dart';
import 'animator/animation_page_slide.dart';
import 'page_painter.dart';
import 'view_model_novel_reader.dart';
import 'animator/animation_page_base.dart';
import 'animator/animation_page_simulation_turn.dart';
...
//  Animation<Offset> animation;
  ...
  ReaderPageManager(this.readerHelper);

  void Function(Offset position) onTapUp;

  void setOnTapUp(void Function(Offset position) onTapUp) {
  ...
  void setCurrentTouchEvent(TouchEvent event) {
    ...
    if (event.action == TouchEvent.ACTION_UP ||
      ...
      if(currentTouchData.action == TouchEvent.ACTION_DOWN) {
        onTapUp(currentTouchData.touchPos);
        return ;
      }
        ...
        ..addStatusListener((status) {
          switch (status) {
            ...
            case AnimationStatus.completed:
              ...
              print('currentState  5555   $currentState');
            ...
            case AnimationStatus.reverse:
              currentState = STATE.STATE_ANIMATING;
              print('currentState  66666   $currentState');
              break;
          }
        });
    }
  ...
  bool shouldRepaint(CustomPainter oldDelegate, PagePainter currentDelegate) {
    ...
    PagePainter oldPainter = (oldDelegate as PagePainter);
  ...
  void setAnimationController(AnimationController animationController) {
    ...
    if (TYPE_ANIMATION_SLIDE_TURN == currentAnimationType) {
      animationController
        ..addListener(() {
          currentState = STATE.STATE_ANIMATING;
          print('currentState  1111   $currentState');
          canvasKey.currentContext?.findRenderObject()?.markNeedsPaint();
        ...
        ..addStatusListener((status) {
          switch (status) {
            ...
            case AnimationStatus.completed:
              currentState = STATE.STATE_IDE;
              print('currentState  2222   $currentState');
              currentAnimationPage
                  .onTouchEvent(TouchEvent(TouchEvent.ACTION_UP, Offset(0, 0)));
              currentTouchData = TouchEvent(TouchEvent.ACTION_UP, Offset(0, 0));
              break;
            ...
            case AnimationStatus.reverse:
              currentState = STATE.STATE_ANIMATING;
              print('currentState  3333   $currentState');
              break;
          }
        });
    }
  ...
  void tapNextPage() {
    switch (currentAnimationType) {
      case TYPE_ANIMATION_SIMULATION_TURN:
        if(currentAnimationPage is SimulationTurnPageAnimation) {
          SimulationTurnPageAnimation animation = currentAnimationPage;
          animation.isTurnToNext=true;
          startConfirmAnimation();
        }
        break;
      case TYPE_ANIMATION_COVER_TURN:
        if(currentAnimationPage is CoverPageAnimation) {
          CoverPageAnimation animation = currentAnimationPage;
          animation.isTurnNext=true;
          startConfirmAnimation();
        }
        break;
      case TYPE_ANIMATION_SLIDE_TURN:
