...
class SimulationTurnPageAnimation extends BaseAnimationPage {
  ...
  void onDraw(Canvas canvas) {
    if (isStartAnimation && (mTouch.dx != 0 && mTouch.dy != 0)) {
      ...
      var targetPicture=readerViewModel?.getCurrentPage()?.pageImage;
      if(targetPicture!=null) {
        canvas.drawImage(targetPicture, Offset.zero, Paint());
      }
    }
    isStartAnimation = false;
  }
...
//
  ...
  void drawTopPageBackArea(Canvas canvas) {
    ...
    canvas.drawPaint(Paint()..color = Color(SettingManager.instance.getFontBackGround()));
...
//    canvas.drawPaint(Paint()..color =Colors.blue);
    canvas.drawPaint(Paint()..color = Color(SettingManager.instance.getFontBackGround() & 0xAAFFFFFF));
